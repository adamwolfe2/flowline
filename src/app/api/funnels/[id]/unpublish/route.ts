...
import { unpublishFunnel } from "@/db/queries/funnels";
...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const funnel = await unpublishFunnel(id, userId);
    ...
    logger.error("POST /api/funnels/[id]/unpublish error:", { error: error instanceof Error ? error.message : String(error) });
